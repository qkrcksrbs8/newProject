...
$(function(){
	...
	$('#updateReportBtn').on('click', function(){
		var report_title = $('#insertTitle').val();
		var report_content = $('#insertContent').val();
		
		if(report_title.length < 1){
			
			alert('제목을 입력하세요.');
			return false;
		
		}else if(report_content.length < 1){
		
			alert('내용을 입력하세요.');
			return false;
		
		}else if(getByteB(report_title) > 25){
			
			alert('제목은 25byte 이하로 작성해주세요.');
			return false;
			
		}else if(getByteB(report_content) > 200){
		...
		if( special_pattern.test(report_title) == true ){
		    alert('제목에 특수문자는 사용할 수 없습니다.');
		    return false;
		}
		if( special_pattern.test(report_content) == true ){
	...
	<section id="content">
		<div class="container board-view-con">  
			<div class="board-view-sub board-view-utilbtn">
				<button id="updateReportBtn" class="btn btn-success" type="submit">작성 완료</button>
				<a href="reportList.do"><button class="btn btn-success">작성 취소</button></a>
			</div>
		</div>
	</section>		
