...
$(function(){
	...
	$('#InsertReportBtn').on('click', function(){//글 작성 버튼		
		location.href='reportWrite.do';//글 작성 페이지로 이동		
	});
	...
	<section id="content">
		<div class="container">
			...
			<div class="board-list-wrap borard-list-con">
				...
				<c:forEach var="reportList" items="${reportList}" varStatus="reportNum">
					<ul class="row">
						<li class="col-md-1 col-xs-1">${reportList.rnum} </li> 
						<li class="col-md-6 col-xs-6"><a href="reportDetail.do?report_seq=${reportList.report_seq}">${reportList.report_title}</a></li>
						<li class="col-md-2 col-xs-2">${reportList.created_by }</li>
						<li class="col-md-2 col-xs-2">${reportList.created_date.substring(2,4)}월 ${reportList.created_date.substring(4)}일</li>	
					</ul>
				</c:forEach>
			</div>
		</div>
		...
		<div class="container">
			<div class="row position:relative;">
				<div class="col-md-12">
					...
					<button id="InsertReportBtn" class="btn btn-success" style="margin-top: 50px">글작성</button>  			
				</div>
			</div>
		</div>
	</section>		
		...
		<div class="container">
			<div class="row">
				<div class="col-12 col-md-10  nopadding">
					<form action="reportList.do" name="search" method="POST">
						<div class="form-row">
