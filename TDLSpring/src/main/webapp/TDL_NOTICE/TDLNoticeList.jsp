		...
		<div class="container">
			...
			<div class="board-list-wrap borard-list-con">
				...
				<c:forEach var="article" items="${list}">
					<ul class="row">
						<li class="col-md-1 col-xs-1"><c:out value="${article.TN_num}" /> </li><!-- <c:set var="number" value="${number-1}" /> -->
						<li class="col-md-6 col-xs-6"><a href="/TDLSpring/TDL_NOTICE/TDLNoticeContent.do?TN_num=${article.TN_num}&TN_id=${article.TN_id}">${article.TN_title}</a></li>
						<li class="col-md-2 col-xs-2">${article.TN_id }</li>
						<li class="col-md-2 col-xs-2">${article.TN_date.substring(0,8)}</li>	
		...
		<div class="row">
			...
			<%-- 		
				<c:if test="${TU_id	!=null }">
					 --%>
						<a href="/TDLSpring/TDL_NOTICE/TDLNoticeWrite.do">
			...
			<div class="col-12 col-md-10  nopadding">
				<form action="/TDLSpring/TDL_NOTICE/TDLNoticeList.do" name="search" method="get" onsubmit="return searchCheck()">
					<div class="form-row">
						<div class="form-group  col-4 col-md-3 offset-md-4">
							<div class="white-section">
								<select id="inputState" name="keyField" class="form-control selectpicker">					
									...
									<option value="TN_content">제목+본문</option>
									<option value="TN_id">작성자</option>
								</select> 
							</div>
						</div>
